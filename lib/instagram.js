async function getInstagramUserId(username, sessionId) {
  ...
  if (!response.ok) {
    if (response.status === 401 || response.status === 403) {
      throw new Error("Instagram session expired or invalid. Please check your sessionid cookie.");
    }
...
async function getInstagramRelation(userId, relationType, sessionId, limit = 500) {
  let list = [];
  let nextMaxId = "";
  let hasNext = true;

  while (hasNext && list.length < limit) {
    const url = `https://www.instagram.com/api/v1/friendships/${userId}/${relationType}/?count=100${nextMaxId ? `&max_id=${nextMaxId}` : ""}`;
    const response = await fetch(url, {
      headers: {
        "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36",
        "Cookie": `sessionid=${sessionId}`,
