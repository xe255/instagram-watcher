...
const RATE_LIMIT = {
  ...
  // feedback_required = session-level anti-spam flag; needs hours, not minutes
  spamBackoffMinutes: [120, 240, 360]
};
...
function isFeedbackRequiredError(errorMessage) {
  return errorMessage && errorMessage.includes("feedback_required");
}

function registerRateLimitHit(db, errorMessage) {
  const watcher = getWatcher(db);
  const isSpamFlag = isFeedbackRequiredError(errorMessage);
  ...
  if (isSpamFlag) {
    ...
    addLog(db, "error", "Session flagged — watcher auto-paused",
      `Instagram anti-spam block detected. Backoff ${RATE_LIMIT.spamBackoffMinutes[step] / 60}h. ` +
      `Log into the admin account in a real browser/app, resolve any challenge, then re-start the watcher.`);
  } else {
...
async function performSync({ manual = false, source = "scheduler" } = {}) {
  ...
  try {
    ...
    if (watcherStatus.adminSessionId) {
      // Real Instagram scraping — pass cached userId to skip the profile fetch when possible
      ...
      // Persist resolved userId so future syncs skip web_profile_info entirely
